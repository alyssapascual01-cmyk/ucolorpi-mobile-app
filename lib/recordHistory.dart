...
class _RecordHistoryPageState extends State<RecordHistoryPage> {
  ...
  // Lightweight evaluation copied from HomePage to decide status
  ...
  String _summary(UrinalysisRecord r) {
    ...
    if (status == 'Normal') return 'All Parameters Within Range.';
    // find first abnormal chemical for an informative summary
    final chemicals = <String, String>{
      'Glucose': r.glucose,
      'Blood': r.blood,
      'Ketone': r.ketone,
      'Protein': r.protein,
    ...
    for (var entry in chemicals.entries) {
      if (entry.value.toLowerCase() != 'negative') {
        return 'Summary: ${entry.key} - ${entry.value}.';
      }
    }
    return 'Summary: Slight deviations detected.';
  }
  ...
  void _applyFilters() {
    ...
    _filtered.sort((a, b) => _sortNewest ? b.date.compareTo(a.date) : a.date.compareTo(b.date));
    setState(() {});
  }

  Widget _gradientButton(String text, VoidCallback onTap, {double width = 160}) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // Gradient header (copied style from SignIn)
          Container(
            ...
            child: SafeArea(
              child: Stack(
                ...
                children: [
                  ...
                  const Center(child: Text('RECORD HISTORY', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18))),
                ],
              ),
            ),
          ),
          ...
          children: [
            ...
            Expanded(
              child: _filtered.isEmpty
                  ? Center(child: Text('No records match your search', style: TextStyle(color: Colors.black54)))
                  : ListView.builder(
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      Text(_summary(r), textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w600)),
                                      const SizedBox(height: 12),
                                      Center(child: _gradientButton('View Full Report', () { Navigator.of(context).push(MaterialPageRoute(builder: (_) => FullReportPage(record: r))); }, width: 160)),
                                    ],
                                  ),
                                ],
                              ),
